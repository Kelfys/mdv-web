...
function bindRulesPanelToggle(main, { open = false } = {}) {
  ...
  const setOpen = (isOpen) => {
    ...
    btn.textContent = isOpen ? t('auth.rulesHide') : t('auth.rulesShow')
  }
...
export async function renderCustomerRegister(main) {
  ...
  main.innerHTML = authLayout(
    ...
    `
      ...
      <p class="form-hint form-hint--center">${t('auth.registerGoogleHint')}</p>
      ...
      <form id="register-form">
        ...
        <div class="form-group">
          <label class="form-label">${t('auth.deliveryPeriodLabel')}</label>
          <select class="form-input" name="delivery_period" required>
            <option value="manha">${deliveryPeriodLabel('manha')}</option>
            <option value="tarde">${deliveryPeriodLabel('tarde')}</option>
            <option value="noite">${deliveryPeriodLabel('noite')}</option>
            <option value="madrugada">${deliveryPeriodLabel('madrugada')}</option>
          </select>
        </div>
...
export async function renderMerchantRegister(main) {
  ...
  if (!user) {
    main.innerHTML = authLayout(
      ...
      `
        ...
        <p class="form-hint form-hint--center">${t('auth.merchantSignupHint')}</p>
        ${renderAuthDivider(t('auth.dividerOrForm'))}
  ...
  if (existing) {
    main.innerHTML = `<div class="empty-state"><h2>${escapeHtml(t('auth.alreadyHasStore'))}</h2><p>${escapeHtml(t('auth.storeStatusLine', { status: existing.status }))}</p><a href="#/dashboard" class="btn btn-primary">${escapeHtml(t('auth.goToPanel'))}</a></div>`
    return
  }
